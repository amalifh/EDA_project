...
#Header, latches, outputs, gates
    def processing_aig(self, filename):
        ...
        for i in range(I):
            lit_input = 2*(i+1)
            base = lit_input >> 1
            node = self.graph.add_node(base, "INPUT")
        ...
        for i, lit in enumerate(outputs): #output is a pointer 
            ...
            base_node = self.graph.add_node(base)
            out_name = f"OUT_{i}"
        ...
        for out_lit, A, B in and_gates:
            out_base = out_lit >> 1
            gate = self.graph.add_node(out_base, "AND")
            gate.delay = init.GATE_DELAYS.get("AND")

            A_base = A >> 1
            A_inv = A & 1
            node_A = self.graph.add_node(A_base)

            B_base = B >> 1 
            B_inv = B & 1
            node_B = self.graph.add_node(B_base)
