...
import circuit_parser2

parser = circuit_parser2.CircuitParser()
entries = parser.parse("simple_circuit.txt")
builder = graph_builder.CircuitBuilder()
graph = builder.building_process(entries)

sorted = topological_sort.toposort_Kahn(graph)

print(arrival_time(graph,sorted))
print(required_time(graph,sorted,10))
